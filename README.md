...
- Thirteen-particle simulated neckline pinned directly around the rear neck arc; the first free rows widen over the trapezius and shoulders, then settle onto a fitted upper-back contact surface, while a thin matching seam and paired side ties terminate at one visible throat brooch—there is no static fabric yoke hiding a shoulder mount
- Allocation-free spatial-hash self-collision with explicit cloth thickness
- Dynamic shoulder, torso, gorget, hip, fitted waist-strap, arm, thigh, knee, lower-leg, and toe-to-heel boot colliders with surface-specific clearances that enclose the animated silhouette; the visible belt is a shallow elliptical leather band rather than torus geometry, and its cloth proxy follows the same rear depth
- One-sided body projection with independently fitted lateral and back-depth radii, so cloth wraps around wide, shallow armor without selecting the character's front surface or inheriting a barrel-shaped proxy gap
...
- Measured procedural character proportions with a 1.8-head shoulder span, a 1.35-head torso whose closed elliptical rings slope continuously from the shoulders into the neck instead of ending in a broad flat cap, a fitted shallow-depth gorget and waist strap, slimmer limbs, an exposed face, a fitted helmet shell with flush brow trim, restrained walk/run bob, and rate-limited eased turning
...
bun run stress:rocks   # optional extended six-rock contact matrix; never a CI gate
...
The renderer-free harness constructs the complete scene graph and dynamically advances cloth, character animation, a moving jump and water landing, center-path rock contact, water drops, footsteps, torches, and mineral systems. It checks finite state, structural error, local fold direction, vertex and cloth-face penetration, pinned-neckline/body clearance, continuous belt-to-boot coverage, jump apex, airborne limb pose, cape follow-through, coupled body/rock contact, landing-ripple emission, self-separation, downward settling, basin depth and rim containment, ripple activity, geometry validity, triangle/draw-call budgets, and stable light counts. It always reports average physics-step cost, but that wall-clock measurement is enforced only by the local command and is telemetry-only in CI. Focused CI tests verify outward-facing, capped speleothem shells; exact transformed rock faces and continuous swept bounds; broad load-bearing rock bases; a real small-rock graze with exact surface contact, below-jump vertical motion, bounded strain, and no crossed cloth rows; genuine rock-edge/cloth-face piercing recovery; zero impulse for clearance-only near misses; a player-width lane beside every mixed-size contact rock; an exact thirteen-particle neck arc whose simulated free rows widen monotonically over the shoulders and join a 4–30 mm settled upper-back contact envelope, plus rendered seam overlap and throat-tie continuity to the front brooch with no static upper-back proxy; rendered torso, gorget, hips, and fitted waist strap contained inside the physical cape-collider union; torso, shoulder, head, and neck proportions; complementary cloth-face collision when every vertex is clear; camera-independent shadow-caster layer membership; suspended-panel wake-up after a 148-degree reversal; distinct walking and running drape; grounded hem and bank traversal; airborne terrain, ceiling, and formation contact; fitted helmet proportions; browser-free clipboard report formatting; eased turning, Shift running, and gait bob; opaque depth-writing inputs with clamped close-camera fade; and nearest-depth ordering for the isolated character layer. The optional local rock-stress command retains the long walk-and-hold traversal against all six authored rocks with bounded per-step motion, structural error, anchor distance, fold direction, and zero face crossing; it is deliberately excluded from CI so runner speed cannot decide correctness.

The visual audit launches an installed Edge or Chrome directly through the DevTools protocol; it does not depend on a browser-testing framework. It verifies that the camera projection matches the viewport on the first frame and after high-density resizes, then drives the deterministic in-app harness through 36 rendered studies, including:

- rear, side, true front, neckline, dedicated true-side neck mount, high-oblique attachment, centered opaque low-front throat-closure, isolated-fade close-rear upper-back clearance, extreme low-side back-contact and waist-strap coverage, aggressive reversal, running, and fully settled cape views;
...
Every push and pull request runs strict TypeScript, renderer-free unit tests, the deterministic full-scene harness, production and Pages builds, and asset-path validation. Pull requests additionally run the 36-study dynamic Windows render audit and upload its PNG/JSON evidence. PR history is checked for merge commits so branches stay reviewable and linear.
